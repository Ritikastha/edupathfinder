...
const BasicInfo = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Create an object with the same key-value pairs
    const id = localStorage.getItem('UserIdHai');

    const fullNameHai = localStorage.getItem('FullNameHai');
    ...
    const formDataObject = {
      ...
      address: address,
      userId: id,
      fullName: fullNameHai,
      currentschool: currentschool
    };

    try {
      const token = localStorage.getItem('token');
      const res = await createBasicinfoApi(formDataObject, token);
